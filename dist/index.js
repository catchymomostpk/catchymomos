...
var MongoStorage = class {
  ...
  async deleteTransactionsByItemAndDate(itemId, date) {
    await this.transactions.deleteMany({
      date,
      items: {
        $elemMatch: { id: itemId }
...
var MemStorage = class {
  ...
  async deleteTransactionsByItemAndDate(itemId, date) {
    const filteredTransactions = Array.from(this.transactions.values()).filter((t) => !(t.date === date && t.items && t.items.some((item) => item.id === itemId)));
    this.transactions.clear();
  ...
  async recalculateSummariesForDate(date) {
...
async function registerRoutes(app2) {
  ...
  app2.delete("/api/transactions/item/:itemId/date/:date", async (req, res) => {
    try {
